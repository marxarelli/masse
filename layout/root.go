...
// LayoutGraph returns a new [state.Graph] for the given [Layout].
func (root *Root) LayoutGraph(name string) (*state.Graph, error) {
	layout, ok := root.Layouts[name]
	if !ok {
		return nil, errors.Errorf("no layout named %q", name)
	}

	return state.NewGraph(root.Chains, layout.Merge())
}
